    ...
    <div id="call" className="gap-5  flex flex-col">
      <div className="md:flex  flex flex-col md:gap-3 md:items-center md:justify-center  md:g-red-800 md:flex-wrap">
        {CallSectionDetails.map((item, index) => (
          <div
            className="flex flex-wrap  w-full p-[1.5rem]  gap-5 border-[1px] border-[var(--greyText)] rounded-lg shadow-lg"
            key={index}
          >
            <div className="bg-[var(--secondary)] w-15 h-15  flex items-center justify-center rounded-[50%] ">
              {" "}
              {index === 0 ? (
                <Phone className="w-8 font-extrabold h-8 text-black" />
              ) : (
                ""
              )}
              {index === 1 ? (
                <Mail className="w-8 font-extrabold h-8 text-black" />
              ) : (
                ""
              )}
              {index === 2 ? (
                <MapPin className="w-8 font-extrabold h-8 text-black" />
              ) : (
                ""
              )}
              {index === 3 ? (
                <Clock className="w-8 font-extrabold h-8 text-black" />
              ) : (
                ""
              )}
              {index === 4 ? (
      ...
      <div className="w-full flex gap-2 p-[1.5rem] bg-red-100 rounded-lg shadow-lg border-l-[5px] border-l-red-500 ">
        <div className="bg-white w-15 h-15  flex items-center justify-center rounded-[50%] ">
          {" "}
          <AlertTriangle className="w-8 font-extrabold h-8 text-red-500" />{" "}
        </div>

        <div className="flex flex-col gap-2 ">
          <div className="font-bold">Emergency Support</div>
          <div className="text-gray-600">
            For urgent technical issues with your solar system:
          </div>
